...
<div class="row">
    <div class="col-12">
        <h2 class="text-center ">AGREGAR UNA ORDEN</h2>
    ...
    <div class="col-12 mt-5">
        <form action="{{ route('order.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group col-12  {{ $errors->has('tax') ? 'has-error' : '' }}">
                ...
                <input type="text" id="tax" name="tax" class="validarletras   form-control{{ $errors->has('tax') ? ' is-invalid' : '' }}"value="{{ old('tax') }}">
        ...
        <div class="table-responsive row">
            <table class=" table table-bordered table-striped table-hover  col-12" id='userTable'>
                ...
                @foreach($products as $id => $products)                 
                        ...
                        <td>
                        <input name="products_price[]" class="d-none" type="checkbox" id="price-{{$id}}" value="{{$products->price}}" />        
                            <input name="products[]" type="checkbox" id="{{$id}}" value="{{$products->id}}" />
